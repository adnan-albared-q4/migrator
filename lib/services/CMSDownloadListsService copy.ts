    ...
    */
    ...
    async create(dl: DownloadLists, test: boolean=false){
        ...
        if (dl.relatedDoc?.customFilename === 'online_content' && dl.relatedDoc?.remotePath) {
            ...
            await this._page.evaluate(() => {
                const radioBtn = document.querySelector<HTMLInputElement>('#_ctrl0_ctl19_rbDownloadUrl');
                if (radioBtn) radioBtn.click();
            });

            // Wait for URL field to be visible
            await this._page.waitForSelector('#_ctrl0_ctl19_txtDownloadUrl', { visible: true });

            // Set the URL
            await this._page.evaluate((url: string) => {
                const urlInput = document.querySelector<HTMLInputElement>('#_ctrl0_ctl19_txtDownloadUrl');
                if (urlInput) updateText(urlInput, url);
            }, dl.relatedDoc.remotePath.toString());
        } else if (dl.relatedDoc && dl.relatedDoc.localPath) {
            await this._page.evaluate((path: string) => {
                updateText(document.querySelector('#_ctrl0_ctl19_ctl01_txtDocument'), path);
